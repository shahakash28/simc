...
SHGen<NetIO> *shgenArr[MAX_THREADS];
SHEval<NetIO> *shevalArr[MAX_THREADS];
...
void msi_relu_6_integrate(int party, block delta_used, NetIO* io, uint64_t* inputs, int nrelu, int bitlen, uint64_t* ip_ss, uint64_t* op_ss, uint64_t* op_mss) {
  //Public prime values
  Integer p(bitlen + 1, prime_mod, PUBLIC);
  Integer p_mod2(bitlen, prime_mod/2, PUBLIC);
  Integer zero(bitlen, 0, PUBLIC);
...
void relu_thread_process(int tid, int party, NetIO* io, uint64_t *inputs, int nrelu, int bitlen, uint64_t* ip_ss, uint64_t* op_ss, uint64_t* op_mss, bool isFirst) {
  if(isFirst){
    if(party == ALICE) {
      shgenArr[tid] = new SHGen<NetIO>(io);
    } else {
      shevalArr[tid] = new SHEval<NetIO>(io);
    }
  }
  ...
  if(party == ALICE) {
    shgenArr[tid]->setup_execution_env();
    delta_used = shgenArr[tid]->delta_used;
    msi_relu_6_integrate(party, delta_used, shgenArr[tid]->io, inputs, nrelu, bitlen, ip_ss, op_ss, op_mss);
  } else {
    shevalArr[tid]->setup_execution_env();
    msi_relu_6_integrate(party, delta_used, shevalArr[tid]->io, inputs, nrelu, bitlen, ip_ss, op_ss, op_mss);
  }
  cout<<"Part 2"<<endl;
}
...
void setup_gen_mult_thread(int tid, SHGen<NetIO> *shgenObj, NetIO* ioObj) {
  shgenObj = new SHGen<NetIO>(ioObj);
}

void setup_eval_mult_thread(int tid, SHEval<NetIO> *shevalObj, NetIO* ioObj) {
  shevalObj = new SHEval<NetIO>(ioObj);
}
...
int main(int argc, char** argv) {
  ...
  std::thread setup_threads[num_threads];
  cout<<"Before Setup"<<endl;
  for(int i=0; i<num_threads; i++) {
  if(party == ALICE) {
    setup_threads[i] = std::thread(setup_gen_mult_thread, i, shgenArr[i], ioArr[i]);
  } else {
    setup_threads[i] = std::thread(setup_eval_mult_thread, i, shevalArr[i], ioArr[i]);
  }
  }
  ...
  if(choice_nn == MINIONN) {
    len = *(&MINIONN_RELUS+1)-MINIONN_RELUS;
  } else {
    len = *(&CIFAR10_RELUS+1)-CIFAR10_RELUS;
  }
  bool isFirst = true;
  for(int i=0; i < len; i++) {
    cout<<"Layer "<<i<<endl;
    for(int j=0; j< num_threads; j++) {
      cout<<"Thread 1"<<endl;
      uint64_t num_relu_layer;
      if(choice_nn == MINIONN) {
        num_relu_layer = MINIONN_RELUS[i];
      } else {
        num_relu_layer = CIFAR10_RELUS[i];
      }
      uint64_t nr_per_thread = num_relu_layer/num_threads;
      uint64_t r = num_relu_layer % num_threads;
      uint64_t actual_per_thread;
      if(j ==num_threads-1)
        ...
        actual_per_thread = nr_per_thread;
      uint64_t offset = prev_ctr + j*nr_per_thread;
      relu_threads[i] = std::thread(relu_thread_process, j, party, ioArr[j], inputs+offset, actual_per_thread, bitlen, ip_ss+offset, op_ss+offset, op_mss+offset, isFirst);
    }
    //isFirst = false;
    if(choice_nn == MINIONN) {
      prev_ctr += MINIONN_RELUS[i];
  ...
  /*for(int i=0; i<num_threads; i++) {
    if(choice_nn == MINIONN) {
      relu_threads[i] = std::thread(thread_process, i, party, inputs, nrelu, bitlen, ip_ss, op_ss, op_mss);
    } else {
      relu_threads[i] = std::thread(thread_process_1, i, party, inputs, nrelu, bitlen, ip_ss, op_ss, op_mss);
    }
  }*/
