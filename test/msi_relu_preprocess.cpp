...
void msi_relu_6(int party, NetIO* io, uint64_t* inputs, uint64_t* rand_vals, int nrelu, int bitlen, uint64_t* ip_mss, uint64_t* rand_mss, uint64_t* op_ss) {
  //Public prime values
  ...
  Integer *Z = new Integer[nrelu];
  for(int i=0; i < nrelu; i++) {
    Z[i] = Integer(bitlen+1, rand_vals, ALICE);
  }

  Integer *R = new Integer[nrelu];
  for(int i=0; i<nrelu; i++) {
    R[i] = Integer(bitlen+1, rand_vals, BOB);
  }

  Integer *S = new Integer[nrelu];
  Integer *XR = new Integer[nrelu];
  Integer *T = new Integer[nrelu];

  //Check if Bob's share is < p
  Bit *res = new Bit[nrelu];
  for(int i=0; i < nrelu; ++i)
    res[i] = Y[i] > p;

  Bit *rres = new Bit[nrelu];
  for(int i=0; i < nrelu; ++i)
    rres[i] = R[i] > p;
  ...
  for(int i=0; i < nrelu; ++i) {
   ...
   //s1.resize(s1.size()+1);
    ...
    Integer relu_res = relu_s.select(is_greater_than_six, six);

    Integer intermediate_share = relu_res + R[i];
    Integer intermediate_share_m = intermediate_share - p;

    borrow_bit = intermediate_share_m[intermediate_share_m.size()-1];
    Integer intermediate_share_modp = intermediate_share_m.select(borrow_bit, intermediate_share);

    Integer res = intermediate_share_modp + Z[i];
    Integer res_m = res - p;
    borrow_bit = res_m[res_m.size()-1];
    Integer res_modp = res_m.select(borrow_bit, res);
  ...
  uint64_t *ip_mcts = (uint64_t *)malloc(nrelu*bitlen*2*sizeof(uint64_t));

  uint64_t *r_mcts = (uint64_t *)malloc(nrelu*bitlen*2*sizeof(uint64_t));

  uint64_t *op_cts = (uint64_t *)malloc(nrelu*bitlen*2*sizeof(uint64_t));

  uint64_t *ipm_pack_table = (uint64_t *)malloc(pack_size*sizeof(uint64_t));
  uint64_t *rm_pack_table = (uint64_t *)malloc(pack_size*sizeof(uint64_t));
  uint64_t *op_pack_table = (uint64_t *)malloc(pack_size*sizeof(uint64_t));

  if(party == ALICE) {

    create_ciphertexts(S, delta_used, ip_mcts, ip_mss, bitlen, nrelu, mac_key, 1);
    create_ciphertexts(R, delta_used, r_mcts, rand_mss, bitlen, nrelu, mac_key, 1);
    create_ciphertexts(T, delta_used, op_cts, op_ss, bitlen, nrelu, 1, 1);

    pack_decryption_table(ipm_pack_table, ip_mcts, pack_size, batch_size, bitlen);
    pack_decryption_table(rm_pack_table, r_mcts, pack_size, batch_size, bitlen);
    pack_decryption_table(op_pack_table, op_cts, pack_size, batch_size, bitlen);

    //cout<<"First element (meth):"<<ip_pack_table[0]<<endl;
    io->send_data(ipm_pack_table, sizeof(uint64_t) * pack_size);
    io->send_data(rm_pack_table, sizeof(uint64_t) * pack_size);
    io->send_data(op_pack_table, sizeof(uint64_t) * pack_size);
  } else {
    io->recv_data(ipm_pack_table, sizeof(uint64_t) * pack_size);
    io->recv_data(rm_pack_table, sizeof(uint64_t) * pack_size);
    io->recv_data(op_pack_table, sizeof(uint64_t) * pack_size);

    unpack_decryption_table(ipm_pack_table, ip_mcts, pack_size, batch_size, bitlen);
    unpack_decryption_table(rm_pack_table, r_mcts, pack_size, batch_size, bitlen);
    unpack_decryption_table(op_pack_table, op_cts, pack_size, batch_size, bitlen);
    //cout<<"First element (meth):"<<ip_pack_table[0]<<endl;

    decrypt_ciphertexts(S, ip_mcts, ip_mss, bitlen, nrelu, 1);
    decrypt_ciphertexts(T, r_mcts, rand_mss, bitlen, nrelu, 0);
    decrypt_ciphertexts(T, op_cts, op_ss, bitlen, nrelu, 1);
  }
}
...
void parse_arguments(int argc, char**arg, int *party, int *port, int *bitlen, int *nrelu) {
  *party = atoi (arg[1]);
  ...
  if(argc < 8) {
    ...
    run_all = (bool)(atoi(arg[7]));
  }

  if(argc < 9) {
    //
  } else {
    num_threads = atoi(arg[8]);
  }
}

void thread_process(int tid, int party, uint64_t* inputs, uint64_t* rand_vals, int nrelu, int bitlen, uint64_t* ip_mss, uint64_t* rand_mss, uint64_t* op_ss) {
  setup_semi_honest(ioArr[tid], party);
  int prev_ctr=0;
  int len = *(&MINIONN_RELUS+1)-MINIONN_RELUS;
  for(int i=0; i<len; i++) {
    uint64_t num_relu_layer = MINIONN_RELUS[i];
    ...
    msi_relu_6(party, ioArr[tid], inputs+offset, rand_vals+offset, actual_per_thread, bitlen, ip_mss+offset, rand_mss+offset, op_ss+offset);
...
void thread_process_1(int tid, int party, uint64_t* inputs, uint64_t* rand_vals, int nrelu, int bitlen, uint64_t* ip_mss, uint64_t* rand_mss, uint64_t* op_ss) {
  setup_semi_honest(ioArr[tid], party);
  int prev_ctr=0;
  int len = *(&CIFAR10_RELUS+1)-CIFAR10_RELUS;
  for(int i=0; i<len; i++) {
...
int main(int argc, char** argv) {
  ...
  uint64_t* rand_vals = (uint64_t *)malloc(nrelu*sizeof(uint64_t));
  for(int i = 0; i < nrelu; ++i)
    rand_vals[i] = distr(eng)%prime_mod;

  uint64_t *ip_mss = (uint64_t *)malloc(nrelu*sizeof(uint64_t));
  uint64_t *rand_mss = (uint64_t *)malloc(nrelu*sizeof(uint64_t));
  ...
  for(int i=0; i<num_threads; i++) {
    if(choice_nn == MINIONN) {
      relu_threads[i] = std::thread(thread_process, i, party, inputs, rand_vals, nrelu, bitlen, ip_mss, rand_mss, op_ss);
    } else {
      relu_threads[i] = std::thread(thread_process_1, i, party, inputs, rand_vals, nrelu, bitlen, ip_mss, rand_mss, op_ss);
    }
  }
